...
type
  ...
  TXMLRTTISerializerOption = (
    xsoWriteDefaultValues, //write object properties with default values
    xsoWriteReadOnlyProperties); //write read only object properties
  TXMLRTTISerializerOptions = set of TXMLRTTISerializerOption;
  ...
  private
    ...
    fOptions: TXMLRTTISerializerOptions;
  private
  ...
  public
    ...
    //custom options
    property Options: TXMLRTTISerializerOptions read fOptions write fOptions;
    //csOXml:    <MyCollection><_oxmlcollection><i>...</i><i>...</i></_oxmlcollection></MyCollection>
    //csOmniXML: <MyCollection><TColItem>...</TColItem><TColItem>...</TColItem></MyCollection>
...
begin
  ...
  case aType.TypeKind of
    ...
    begin
      if not aIsDefaultValue or (xsoWriteDefaultValues in fOptions) then
      case aType.TypeKind of
    ...
    begin
      ...
      else if (aType.Handle = System.TypeInfo(TDate)) then
    ...
    begin
      ...
      if (xProperty.Visibility in xVisibility) and
         (xProperty.IsWritable or xProperty.PropertyType.IsInstance or (xsoWriteReadOnlyProperties in fOptions))
      then
      begin
  ...
  case aType.TypeKind of
    ...
    case aType.TypeKind of
      ...
      begin
        ...
        end else if (aType.Handle = System.TypeInfo(TTime)) then
        begin
          xResult := ISOTryStrToTime(xStrValue, TDateTime(xDoubleValue));
          if xResult then
            xExtendedValue := xDoubleValue;
        end else if (aType.Handle = System.TypeInfo(TDate)) then
        begin
