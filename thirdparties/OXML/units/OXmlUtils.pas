...
function ISOIntToStr(const aValue: Integer): string; overload; {$IFDEF O_INLINE}inline;{$ENDIF}
function ISOIntToStr(const aValue: Int64): string; overload; {$IFDEF O_INLINE}inline;{$ENDIF}
...
function ISODateTimeToStr(const aDateTime: TDateTime; const aUseT: Boolean = True): string; {$IFDEF O_INLINE}inline;{$ENDIF}
...
function ISOStrToInt64(const aString: string): Int64; {$IFDEF O_INLINE}inline;{$ENDIF}
...
function ISOStrToInt64Def(const aString: string; const aDefValue: Int64): Int64; {$IFDEF O_INLINE}inline;{$ENDIF}
...
function ISOTryStrToInt64(const aString: string; var outValue: Int64): Boolean; {$IFDEF O_INLINE}inline;{$ENDIF}
...
function ISOIntToStr(const aValue: Integer): string;
begin
  Result := IntToStr(aValue);
end;

function ISOIntToStr(const aValue: Int64): string;
begin
  Result := IntToStr(aValue);
end;
...
function ISODateTimeToStr(const aDateTime: TDateTime;
  const aUseT: Boolean): string;
begin
  if aUseT then
...
begin
  if not ISOTryStrToBool(aString, Result{%H-}) then
    raise EConvertError.CreateFmt('"%s" is not a valid ISO boolean.', [aString]);
end;

function ISOStrToInt(const aString: string): Integer;
begin
  if not ISOTryStrToInt(aString, Result{%H-}) then
    raise EConvertError.CreateFmt('"%s" is not a valid ISO integer.', [aString]);
end;

function ISOStrToInt64(const aString: string): Int64;
begin
  if not ISOTryStrToInt64(aString, Result{%H-}) then
    raise EConvertError.CreateFmt('"%s" is not a valid ISO int64.', [aString]);
end;

function ISOStrToFloat(const aString: string): Extended;
begin
  if not ISOTryStrToFloat(aString, Result{%H-}) then
    raise EConvertError.CreateFmt('"%s" is not a valid ISO float.', [aString]);
end;

function ISOStrToDate(const aString: string): TDateTime;
begin
  if not ISOTryStrToDate(aString, Result{%H-}) then
    raise EConvertError.CreateFmt('"%s" is not a valid ISO date.', [aString]);
end;

function ISOStrToDateTime(const aString: string): TDateTime;
begin
  if not ISOTryStrToDateTime(aString, Result{%H-}) then
    raise EConvertError.CreateFmt('"%s" is not a valid ISO datetime.', [aString]);
end;

function ISOStrToTime(const aString: string): TDateTime;
begin
  if not ISOTryStrToTime(aString, Result{%H-}) then
    raise EConvertError.CreateFmt('"%s" is not a valid ISO time.', [aString]);
end;
...
function ISOStrToInt64Def(const aString: string; const aDefValue: Int64): Int64;
begin
  if not ISOTryStrToInt64(aString, Result{%H-}) then
    Result := aDefValue;
end;
...
function ISOTryStrToInt64(const aString: string; var outValue: Int64): Boolean;
begin
  Result := TryStrToInt64(aString, outValue);
end;
